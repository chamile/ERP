...
describe('Testing OrderService.js api calls', () => {
  ...
  test('should get order list', async (done) => {
    const data = await API.get(`api/biz/orders`);
    expect(data.data).toBeType('array');
    done();
  });

  test('should get order details', async (done) => {
    const data = await API.get(`api/biz/orders/1?expand=customer, currencycode,CreditDays, Items, Items.CurrencyCode, Items.VatType, DefaultDimensions.Project, DefaultSeller`);
