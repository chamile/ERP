...
describe('Testing CurrencyService.js api calls', () => {
  ...
  test('should get currency codes', async (done) => {
    const data = await API.get(`api/biz/currencycodes`);
    expect(data.data).toBeType('array');
    done();
  });

  test('should get currency code details', async (done) => {
    const data = await API.get(`api/biz/currencycodes/1`);
    expect(data.data).toBeType('object');
    expect(data.data.Code).toEqual('NOK');
    expect(data.data.ID).toEqual(1);
    done();
  });

  test('should get vattypes', async (done) => {
    const data = await API.get(`api/biz/vattypes?filter=Visible eq true`);
    expect(data.data).toBeType('array');
    done();
  });
});
