...
describe('Testing ProductService.js api calls', () => {
  ...
  test('should get quotes list', async (done) => {
    const data = await API.get(`api/biz/quotes?expand=customer, currencycode`);
    expect(data.data).toBeType('array');
    done();
  });

  test('should get notification List', async (done) => {
    const data = await API.get(`api/biz/quotes/1?expand=customer, CreditDays,currencycode, Items, Items.CurrencyCode, Items.VatType, DefaultDimensions.Project, DefaultSeller`);
