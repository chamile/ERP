...
describe('Testing ProjectService.js api calls', () => {
  ...
  test('should get project list', async (done) => {
    const data = await API.get(`api/biz/projects/`);
    expect(data.data).toBeType('array');
    done();
  });

  test('should get currency code details', async (done) => {
    const data = await API.get(`api/biz/dimensions/?filter=ProjectID eq 1`);
    expect(data.data).toBeType('array');
    done();
  });
