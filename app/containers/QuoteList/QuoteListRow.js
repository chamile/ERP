...
export const QuoteListRow = ({ item, index, onSelect }) => {
  ...
  return (
    <Touchable onPress={() => onSelect(item)} key={item.ID} style={styles.button}>
      ...
      <View style={styles.mainTextContainer}>
        <Text style={styles.infoTextMain} numberOfLines={1} ellipsizeMode={'tail'}>{item.CustomerName ? item.CustomerName.trim() : i18n.t('QuoteList.QuoteListRow.unknown')}</Text>
        <View style={styles.detailsContainer}>
          <View style={styles.infoLeft}>

            <Text style={styles.infoText}>{`${item.Customer ? item.Customer.CustomerNumber : i18n.t('QuoteList.QuoteListRow.customer') + ' -'}`}</Text>
            <Text style={styles.infoTextSubLeft}>{`${i18n.t('QuoteList.QuoteListRow.quote')} ${item.QuoteNumber ? '- ' + item.QuoteNumber : '-'} ${item.QuoteDate ? 'on ' + item.QuoteDate : ''}`}</Text>
            <Text style={styles.infoTextSubLeftSecond}>{`${item.ValidUntilDate ? i18n.t('QuoteList.QuoteListRow.due') + item.ValidUntilDate : ''}`}</Text>
          </View>
          <View style={styles.infoRight}>
            <View style={styles.amountContainer}>
              <Text>
                <Text style={styles.amount}>{FormatNumber(item.TaxInclusiveAmountCurrency, i18n.language)}</Text>
