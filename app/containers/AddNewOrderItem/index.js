...
class AddNewOrderItem extends Component {
  static navigationOptions = ({ navigation }) => {
    return {
      title: navigation.state.params.edit ? i18n.t('AddNewOrderItem.index.editItem') : i18n.t('AddNewOrderItem.index.addItem'),
      headerRight: <HeaderTextButton navigate={navigation.navigate} onPress={() => { Keyboard.dismiss(); navigation.state.params.edit ? navigation.state.params.editItemOnOrder() : navigation.state.params.addItemToOrder(); }} text={navigation.state.params.edit ? i18n.t('AddNewOrderItem.index.save') : i18n.t('AddNewOrderItem.index.add')} position="right" isActionComplete={navigation.state.params.disableActionButton ? navigation.state.params.disableActionButton : false} />,
      headerLeft: <HeaderTextButton navigate={navigation.navigate} onPress={() => { Keyboard.dismiss(); navigation.goBack(); }} text={i18n.t('AddNewOrderItem.index.cancel')} position="left" isActionComplete={navigation.state.params.disableActionButton ? navigation.state.params.disableActionButton : false} />,
  ...
  constructor(props) {
    ...
    this.editItemOnOrder = this.editItemOnOrder.bind(this);
  ...
  componentDidMount() {
    this.props.navigation.setParams({ addItemToOrder: this.addItemToOrder, disableActionButton: false, editItemOnOrder: this.editItemOnOrder });
  ...
  onDiscountChange() {
    ...
    if (DiscountPercent !== false) {
      ...
      if (this.state.productDetails.DiscountPercentDisplay == '') {
        ...
        this.setState({ errors: { ...this.state.errors, DiscountPercent: i18n.t('AddNewOrderItem.index.discountFormatError') } });
      }
    }
  }
  ...
  onPriceChange() {
    if (this.state.productDetails.PriceExVatCurrencyDisplay !== '') {
      ...
      if (PriceExVatCurrency !== false) {
        ...
        this.setState({ errors: { ...this.state.errors, PriceExVatCurrency: i18n.t('AddNewOrderItem.index.priceFormatError') } });
  ...
  onPriceIncVatChange() {
    ...
    if (PriceIncVatCurrency !== false) {
      ...
      if (this.state.productDetails.PriceIncVatCurrencyDisplay == '') {
        ...
        this.setState({ errors: { ...this.state.errors, PriceIncVatCurrency: i18n.t('AddNewOrderItem.index.priceFormatError') } });
      }
    }
  }
  ...
  onNumberChange() {
    ...
    if (NumberOfItems) {
      ...
      this.setState({ errors: { ...this.state.errors, NumberOfItems: i18n.t('AddNewOrderItem.index.numberOfItemsFormatError') } });
    }
  }
  ...
  fetchProductDetails() {
    ProductService.getProduct(this.props.navigation.state.params.productID, this.props.company.selectedCompany.Key)
      ...
      .then(response => {
        ...
        this.handleProductErrors(err.problem, this.componentDidMount.bind(this), i18n.t('AddNewOrderItem.index.fetchingProductError'));
      });
  }
  ...
  addItemToOrder() {
    ...
    setTimeout(() => {
      if (this.isValid()) {
        this.props.navigation.setParams({ disableActionButton: true });
        OrderService.addNewItem(this.props.navigation.state.params.orderID, this.getItemDetails(), this.props.company.selectedCompany.Key)
          .then((res) => {
            ...
            Analytics.trackEvent(AnalyticalEventNames.EDIT_ITEM, { entity: 'Customer order item' });
          })
          .catch((err) => {
            this.startAnimation(true);
            this.handleProductErrors(err.problem, this.addItemToOrder, i18n.t('AddNewOrderItem.index.addItemError'));
            this.props.navigation.setParams({ disableActionButton: false });
          });
      }
      else {
  ...
  editItemOnOrder() {
    ...
    setTimeout(() => {
      if (this.isValid()) {
        this.props.navigation.setParams({ disableActionButton: true });
        OrderService.addNewItem(this.props.navigation.state.params.orderID, this.getItemDetails(true), this.props.company.selectedCompany.Key)
          .then((res) => {
            this.startAnimation(true);
            this.props.navigation.state.params.refreshData();
            this.props.navigation.goBack();
            this.props.navigation.setParams({ disableActionButton: false });
            Analytics.trackEvent(AnalyticalEventNames.CREATE_ITEM, { entity: 'Customer order item' });
          })
          .catch((err) => {
            let errorMsg = i18n.t('AddNewOrderItem.index.editItemError');
            if (err.data && err.data.ErrorsCount && err.data.ErrorsCount > 0) {
              errorMsg = err.data.Messages[0].Message;
              Alert.alert('ERROR', errorMsg);
            } else {
              this.handleProductErrors(err.problem, this.addItemToOrder, errorMsg);
            }
            this.startAnimation(true);
            this.props.navigation.setParams({ disableActionButton: false });
          });
  ...
  isValid() {
    ...
    if ((!this.state.productDetails.PriceExVatCurrency && DeFormatNumber(this.state.productDetails.PriceExVatCurrencyDisplay) !== 0) || (DeFormatNumber(this.state.productDetails.PriceExVatCurrencyDisplay) === false)) {
      if (this.state.productDetails.PriceExVatCurrencyDisplay == '' && this.props.navigation.state.params.edit) {
        this.resetData('PriceExVatCurrencyDisplay');
      }
      else {
  ...
  setSpecialDiscountForCustomer() {
    ...
    if ((SumTotalIncVatCurrency !== false) && !isNaN(this.state.productDetails.NumberOfItems)) {
      ...
      if (this.state.productDetails.Sum == '') {
        ...
        this.setState({ errors: { ...this.state.errors, Sum: i18n.t('AddNewOrderItem.index.sumFormatError') } });
      }
    }
  }
  ...
  handleProductErrors(problem = 'CLIENT_ERROR', caller, userErrorMessage = i18n.t('AddNewOrderItem.index.somethingWrong')) {
      ...
      <ViewWrapperAsync withFade={true} withMove={true} isReady={!this.state.isLoading} fromBackgroundStyle={styles.fromBackgroundStyle} toBackgroundStyle={styles.toBackgroundStyle} loaderPosition={'nav_tabs'} statusBarColor={theme.SECONDARY_STATUS_BAR_COLOR}>
        ...
        >
          ...
          <View style={styles.animatedPanelText}>
            <Text>{`${this.props.navigation.state.params.edit ? i18n.t('AddNewOrderItem.index.updatingItem') : i18n.t('AddNewOrderItem.index.savingItem')} ...`}</Text>
          </View>
        </Animated.View>
        ...
        <ScrollView keyboardDismissMode={'interactive'} key={'scrollView'} ref={ref => this._scrollview = ref} horizontal={false} keyboardShouldPersistTaps={Platform.OS === 'android' ? 'handled' : 'never'}>
          <ImageCarousel
            ...
            viewModeOnly={true}
            noImageTitle={i18n.t('AddNewOrderItem.index.noImages')}
          ...
          <View style={styles.infoContainer}>
            <InlineValidationInputField
              title={i18n.t('AddNewOrderItem.index.SKU')}
              placeHolder={'SKU'}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewOrderItem.index.product')}
              placeHolder={i18n.t('AddNewOrderItem.index.productName')}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewOrderItem.index.number')}
              placeHolder={'0'}
              isKeyboardInput={true}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewOrderItem.index.unit')}
              placeHolder={'Mth.'}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewOrderItem.index.price') + ` (${this.state.productDetails.CurrencyCode.Code})`}
              placeHolder={'0.00'}
              ...
              value={this.state.productDetails.PriceExVatCurrencyDisplay.toString()}
              error={this.state.errors.PriceExVatCurrency}
            />
            <InlineValidationInputField
              title={i18n.t('AddNewOrderItem.index.priceIncVat') + ` (${this.state.productDetails.CurrencyCode.Code})`}
              placeHolder={'0.00'}
              isKeyboardInput={true}
              keyboardType={'numeric'}
              ...
              value={this.state.productDetails.PriceIncVatCurrencyDisplay.toString()}
              error={this.state.errors.PriceIncVatCurrency}
            />
            <InlineValidationInputField
              title={i18n.t('AddNewOrderItem.index.vatCode')}
              placeHolder={''}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewOrderItem.index.discount') + ' %'}
              placeHolder={'0%'}
              ...
              value={this.state.productDetails.DiscountPercentDisplay.toString()}
              error={this.state.errors.DiscountPercent}
            />
            <InlineValidationInputField
              title={i18n.t('AddNewOrderItem.index.sum') + ` (${this.state.productDetails.CurrencyCode.Code})`}
              placeHolder={'0.00'}
              isKeyboardInput={true}
              editable={true}
              keyboardType={'numeric'}
              ...
              value={this.state.productDetails.Sum.toString()}
