...
class AddNewCustomerInvoiceItem extends Component {
  static navigationOptions = ({ navigation }) => {
    return {
      title: navigation.state.params.edit ? i18n.t('AddNewCustomerInvoiceItem.index.editItem') : i18n.t('AddNewCustomerInvoiceItem.index.addItem'),
      headerRight: <HeaderTextButton navigate={navigation.navigate} onPress={() => { Keyboard.dismiss(); navigation.state.params.edit ? navigation.state.params.editItemOnCustomerInvoice() : navigation.state.params.addItemToCustomerInvoice(); }} text={navigation.state.params.edit ? i18n.t('AddNewCustomerInvoiceItem.index.save') : i18n.t('AddNewCustomerInvoiceItem.index.add')} position="right" isActionComplete={navigation.state.params.disableActionButton ? navigation.state.params.disableActionButton : false} />,
      headerLeft: <HeaderTextButton navigate={navigation.navigate} onPress={() => { Keyboard.dismiss(); navigation.goBack(); }} text={i18n.t('AddNewCustomerInvoiceItem.index.cancel')} position="left" isActionComplete={navigation.state.params.disableActionButton ? navigation.state.params.disableActionButton : false} />,
  ...
  constructor(props) {
    ...
    this.addItemToCustomerInvoice = this.addItemToCustomerInvoice.bind(this);
    ...
    this.editItemOnCustomerInvoice = this.editItemOnCustomerInvoice.bind(this);
  ...
  componentDidMount() {
    this.props.navigation.setParams({ addItemToCustomerInvoice: this.addItemToCustomerInvoice, disableActionButton: false, editItemOnCustomerInvoice: this.editItemOnCustomerInvoice });
  ...
  onDiscountChange() {
    ...
    if (DiscountPercent !== false) {
      ...
      if (this.state.productDetails.DiscountPercentDisplay == '') {
        ...
        this.setState({ errors: { ...this.state.errors, DiscountPercent: i18n.t('AddNewCustomerInvoiceItem.index.discountFormatError') } });
      }
  ...
  resetData(Key) {
    ...
    if (Key == 'DiscountPercentDisplay') {
      this.setState({ productDetails: { ...this.state.productDetails, DiscountPercent: 0, DiscountPercentDisplay: this.state.localizedZero } }, () => {
        this.calculateTradePrices();
      });
    }
  ...
  onPriceChange() {
    if (this.state.productDetails.PriceExVatCurrencyDisplay !== '') {
      ...
      if (PriceExVatCurrency !== false) {
        ...
        this.setState({ errors: { ...this.state.errors, PriceExVatCurrency: i18n.t('AddNewCustomerInvoiceItem.index.priceFormatError') } });
      }
    }
    else {
  ...
  onPriceIncVatChange() {
    ...
    if (PriceIncVatCurrency !== false) {
      ...
      if (this.state.productDetails.PriceIncVatCurrencyDisplay == '') {
        ...
        this.setState({ errors: { ...this.state.errors, PriceIncVatCurrency: i18n.t('AddNewCustomerInvoiceItem.index.priceFormatError') } });
      }
  ...
  onNumberChange() {
    ...
    if (NumberOfItems) {
      ...
      this.setState({ errors: { ...this.state.errors, NumberOfItems: i18n.t('AddNewCustomerInvoiceItem.index.numberOfItemsFormatError') } });
    }
  }
  ...
  fetchProductDetails() {
    ProductService.getProduct(this.props.navigation.state.params.productID, this.props.company.selectedCompany.Key)
      ...
      .then((response) => {
        ...
        this.handleProductErrors(err.problem, this.componentDidMount.bind(this), i18n.t('AddNewCustomerInvoiceItem.index.fetchingProductError'));
      });
  }
  ...
  addItemToCustomerInvoice() {
    this.startAnimation();
    Keyboard.dismiss();
    this.props.navigation.setParams({ disableActionButton: true });
    setTimeout(() => {
      if (this.isValid()) {
        CustomerInvoiceService.addNewItem(this.props.navigation.state.params.customerInvoiceID, this.getItemDetails(), this.props.company.selectedCompany.Key)
          .then((res) => {
            ...
            Analytics.trackEvent(AnalyticalEventNames.CREATE_ITEM, { entity: 'Customer invoice item' });
          })
          .catch((err) => {
            this.startAnimation(true);
            this.handleProductErrors(err.problem, this.addItemToCustomerInvoice, i18n.t('AddNewCustomerInvoiceItem.index.addItemError'));
            this.props.navigation.setParams({ disableActionButton: false });
          });
      }
      else {
  ...
  editItemOnCustomerInvoice() {
    this.startAnimation();
    Keyboard.dismiss();
    setTimeout(() => {
      if (this.isValid()) {
        this.props.navigation.setParams({ disableActionButton: true });
        CustomerInvoiceService.addNewItem(this.props.navigation.state.params.customerInvoiceID, this.getItemDetails(true), this.props.company.selectedCompany.Key)
          .then((res) => {
            this.startAnimation(true);
            this.props.navigation.state.params.refreshData();
            this.props.navigation.goBack();
            this.props.navigation.setParams({ disableActionButton: false });
            Analytics.trackEvent(AnalyticalEventNames.EDIT_ITEM, { entity: 'Customer invoice item' });
          })
          .catch((err) => {
            let errorMsg = i18n.t('AddNewCustomerInvoiceItem.index.editItemError');
            if (err.data && err.data.ErrorsCount && err.data.ErrorsCount > 0) {
              errorMsg = err.data.Messages[0].Message;
              Alert.alert('ERROR', errorMsg);
            } else {
              this.handleProductErrors(err.problem, this.addItemToCustomerInvoice, errorMsg);
            }
            this.startAnimation(true);
            this.props.navigation.setParams({ disableActionButton: false });
          });
  ...
  isValid() {
    ...
    if (DeFormatNumber(this.state.productDetails.DiscountPercentDisplay) === false) {
      if (this.state.productDetails.DiscountPercentDisplay == '' && this.props.navigation.state.params.edit) {
        ...
        errors.DiscountPercent = i18n.t('AddNewOrderItem.index.discountFormatError');
      }
    ...
    if (!this.state.productDetails.NumberOfItems || (DeFormatNumber(this.state.productDetails.NumberOfItemsDisplay) === false) || DeFormatNumber(this.state.productDetails.NumberOfItemsDisplay) === 0) {
      errors.NumberOfItems = i18n.t('AddNewOrderItem.index.numberOfItemsFormatError');
    }
    if ((!this.state.productDetails.PriceExVatCurrency && DeFormatNumber(this.state.productDetails.PriceExVatCurrencyDisplay) !== 0) || (DeFormatNumber(this.state.productDetails.PriceExVatCurrencyDisplay) === false)) {
      errors.PriceExVatCurrency = i18n.t('AddNewOrderItem.index.priceFormatError');
    }
    if ((!this.state.productDetails.PriceIncVatCurrency && DeFormatNumber(this.state.productDetails.PriceIncVatCurrencyDisplay) !== 0) || (DeFormatNumber(this.state.productDetails.PriceIncVatCurrencyDisplay) === false)) {
      errors.PriceIncVatCurrency = i18n.t('AddNewOrderItem.index.priceFormatError');
    }
    if ((!this.state.productDetails.Sum && DeFormatNumber(this.state.productDetails.Sum) !== 0) || (DeFormatNumber(this.state.productDetails.Sum) === false)) {
      errors.Sum = i18n.t('AddNewOrderItem.index.sumFormatError');
    }
  ...
  selectVatCode() {
    ...
    this.props.navigation.navigate('LookupVatView', {
      data: {
        title: i18n.t('AddNewCustomerInvoiceItem.index.selectVatType'),
        searchCriteria: i18n.t('AddNewCustomerInvoiceItem.index.vatType'),
  ...
  setVATCode(VatType) {
    this.setState({ productDetails: { ...this.state.productDetails, VatType, VatTypeID: VatType.ID } }, () => { this.calculateTradePrices(); });
  }
  ...
  calculateTradePrices() { // logic from https://github.com/unimicro/AppFrontend/blob/develop/src/app/components/sales/salesHelper/TradeItemHelper.ts
    ...
    const PriceIncVatCurrency = this.calculatePriceIncVat(PriceExVatCurrency, this.state.productDetails.VatType.VatPercent);
  ...
  setSpecialDiscountForCustomer() {
    ...
    if ((SumTotalIncVatCurrency !== false) && !isNaN(this.state.productDetails.NumberOfItems)) {
      ...
      if (this.state.productDetails.Sum == '') {
        ...
        this.setState({ errors: { ...this.state.errors, Sum: i18n.t('AddNewCustomerInvoiceItem.index.sumFormatError') } });
      }
  ...
  handleProductErrors(problem = 'CLIENT_ERROR', caller, userErrorMessage = i18n.t('AddNewCustomerInvoiceItem.index.somethingWrong')) {
  ...
  render() {
    return (
      <ViewWrapperAsync withFade={true} withMove={true} isReady={!this.state.isLoading} fromBackgroundStyle={styles.fromBackgroundStyle} toBackgroundStyle={styles.toBackgroundStyle} loaderPosition={'nav_tabs'}>
        <Animated.View
        ...
        >
          ...
          <View style={styles.animatedPanelText}>
            <Text>{`${this.props.navigation.state.params.edit ? i18n.t('AddNewCustomerInvoiceItem.index.updatingItem') : i18n.t('AddNewCustomerInvoiceItem.index.savingItem')} ...`}</Text>
          </View>
        </Animated.View>
        ...
        <ScrollView keyboardDismissMode={'interactive'} key={'scrollView'} ref={ref => this._scrollview = ref} horizontal={false} keyboardShouldPersistTaps={Platform.OS === 'android' ? 'handled' : 'never'}>
          <ImageCarousel
            ...
            viewModeOnly={true}
            noImageTitle={i18n.t('AddNewCustomerInvoiceItem.index.noImages')}
          ...
          <View style={styles.infoContainer}>
            <InlineValidationInputField
              title={i18n.t('AddNewCustomerInvoiceItem.index.SKU')}
              placeHolder={'SKU'}
              isKeyboardInput={true}
              editable={false}
              value={this.state.productDetails.ProductID.toString()}
            />
            <InlineValidationInputField
              title={i18n.t('AddNewCustomerInvoiceItem.index.product')}
              placeHolder={i18n.t('AddNewCustomerInvoiceItem.index.productName')}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewCustomerInvoiceItem.index.number')}
              placeHolder={'0'}
              isKeyboardInput={true}
              ...
              onFocus={this.scrollToBottom}
              value={this.state.productDetails.NumberOfItemsDisplay.toString()}
              error={this.state.errors.NumberOfItems}
            />
            <InlineValidationInputField
              title={i18n.t('AddNewCustomerInvoiceItem.index.unit')}
              placeHolder={'Mth.'}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewCustomerInvoiceItem.index.price') + ` (${this.state.productDetails.CurrencyCode.Code})`}
              placeHolder={'0.00'}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewCustomerInvoiceItem.index.priceIncVat') + ` (${this.state.productDetails.CurrencyCode.Code})`}
              placeHolder={'0.00'}
              isKeyboardInput={true}
              keyboardType={'numeric'}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewCustomerInvoiceItem.index.vatCode')}
              placeHolder={''}
              isKeyboardInput={false}
              onPress={this.selectVatCode}
              value={`${this.state.productDetails.VatType.Name} ${this.state.productDetails.VatPercent != null ? this.state.productDetails.VatPercent : this.calculateVatPercentManually(this.state.productDetails)}%`}
            />
            <InlineValidationInputField
              title={i18n.t('AddNewCustomerInvoiceItem.index.discount') + ' %'}
              placeHolder={'0%'}
            ...
            />
            <InlineValidationInputField
              title={i18n.t('AddNewCustomerInvoiceItem.index.sum') + ` (${this.state.productDetails.CurrencyCode.Code})`}
              placeHolder={'0.00'}
              isKeyboardInput={true}
              editable={true}
              keyboardType={'numeric'}
