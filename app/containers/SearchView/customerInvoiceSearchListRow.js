...
export const CustomerInvoiceSearchListRow = ({ item, index, onSelect }) => {
  ...
  const StatusObj = _processStatus(item.CustomerInvoiceStatusCode);
  ...
  return (
    <Touchable onPress={() => onSelect(item)} key={item.ID} style={styles.button}>
      ...
      <View style={styles.mainTextContainer}>
        <Text style={styles.infoTextMain} numberOfLines={1} ellipsizeMode={'tail'}>{(item.InfoName) ? item.InfoName.trim() : i18n.t('CustomerInvoiceList.CustomerInvoiceListRow.unknown')}</Text>
        <View style={styles.detailsContainer}>
          <View style={styles.infoLeft}>
            ...
            <Text style={styles.infoTextSubLeft}>{`${i18n.t('CustomerInvoiceList.CustomerInvoiceListRow.invoiceDate')} - ${item.CustomerInvoiceInvoiceDate ? moment(item.CustomerInvoiceInvoiceDate).format('YYYY-MM-DD') : i18n.t('CustomerInvoiceList.CustomerInvoiceListRow.notSpecified')}`}</Text>
            <View style={styles.rowContainer}>
              <Text style={styles.infoTextSubLeftDueDate}>{`${i18n.t('CustomerInvoiceList.CustomerInvoiceListRow.dueDate')} - `}</Text>
              <Text style={[styles.infoTextSubLeft, item.CustomerInvoicePaymentDueDate ? moment(item.CustomerInvoicePaymentDueDate).diff(moment(formatDate(new Date())), 'days') < 1 && item.CustomerInvoiceStatusCode !== CustomerInvoiceStatusCodes.PAID ? { color: theme.SCREEN_COLOR_LIGHT_RED } : { color: theme.HINT_COLOR } : {}]}>{item.CustomerInvoicePaymentDueDate ? moment(item.CustomerInvoicePaymentDueDate).format('YYYY-MM-DD') : i18n.t('CustomerInvoiceList.CustomerInvoiceListRow.notSpecified')}</Text>
            </View>
          </View>
          <View style={styles.infoRight}>
            <View style={styles.amountContainer}>
              <Text>
                <Text style={[styles.amount, item.TaxExclusiveAmountCurrency < 0 ? { color: theme.SCREEN_COLOR_LIGHT_RED } : {}]}>{FormatNumber(item.CustomerInvoiceTaxInclusiveAmountCurrency, i18n.language)}</Text>
